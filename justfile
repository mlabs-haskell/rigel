...
cv_hier_db_out_dir := "../step0/context-vectors-compressed"
cv_hier_db_test_out_dir := "../step0/context-vectors-compressed-test"

_cv_hier_db mode out_dir=cv_hier_db_out_dir narrow_factor='16' limit='':
	python3 -m scripts.generate_hier_cv_db \
			--compressor_chkpt "../step0/attention_model.pt" \
			--cv_db_dir "../step0/context-vectors/" \
			--hier_db_dir {{out_dir}} \
...
generate_context_vectors:
	python3 -m scripts.generate_context_vectors \
			...
			--cv_db_folder ../step0/context-vectors \
			--article_list_file all.txt \
			--transformer_position 0
...
generate_tfidf:
	python3 -m scripts.generate_tfidf \
			...
			--out_file ../step0/tfidf.json \
			--cvdb_folder ../step0/context-vectors
...
train_compressor validation='True' epochs='100':
	python3 -m scripts.train_compressor train \
			--checkpoint_file ../step0/attention_model.pt \
			--network_type attention \
			--reduction_factor 4 \
			--cvdb_folder ../step0/context-vectors/ \
			--validation {{validation}} \
			--epochs {{epochs}} \
			--tfidf_file ../step0/tfidf.json
