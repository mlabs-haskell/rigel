...
class IndexedBinaryDB:
    ...
    def write(self, metadata, obj) -> FileSpan:
        ...
        self._write_index(metadata, file_span)
        return file_span
    ...
    def read_full_index(self) -> dict[Any, FileSpan]:
        """
        Returns dict[Metadata, FileSpan].
        """
        self._index_file.seek(0)
        res: dict[Any, FileSpan] = {}
        while True:
            try:
                metadata, file_span = self._read_index()
            except EOFError:
                break
            res[metadata] = file_span
        return res

    def _write_index(self, metadata, file_span: FileSpan) -> None:
    ...
    def _read_index(self) -> tuple[Any, FileSpan]:
